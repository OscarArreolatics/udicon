...
switch ($identificador) {
	...
	case 3:
		...
		$prodspagina=(isset($_GET['prodspagina']))?$_GET['prodspagina']:12;
	...
	case 10:
		...
		$tallaID = (isset($_GET['talla']))? $_GET['talla']: 0;
		...
		if ($cat>0) {
			...
			$txtsubcat=html_entity_decode($row_consultaX['txt']);

			$parentid = $row_consultaX['parent'];

			$consulta3 = $CONEXION -> query("SELECT * FROM productoscat WHERE id = $parentid");
			$row_consulta3 = $consulta3 -> fetch_assoc();
			$txtparcat = html_entity_decode($row_consulta3['txt']);
	...
	case 12:
		...
		$sqlProd="SELECT id FROM productos WHERE estatus = 1 AND preventa = 1 ";
		...
		require_once 'includes/includes.php';
		require_once 'pages/preventa.php';
		break;		
